...
import { getDocLabels, GaneshaIcon, FloralCorner, GoldDivider, FONTS } from './templateUtils';
    ...
    >
      ...
      <div className="text-center relative z-10 mb-6" data-pdf-section="header">
        <div className="flex justify-center mb-1">
          <GaneshaIcon className="w-9 h-9" color={maroon} />
        </div>
        {data.religiousHeading && (
          <p
            className="text-xs font-semibold tracking-wider mb-1"
            style={{ color: maroon, fontFamily: FONTS.marcellus }}
          >
            {data.religiousHeading}
          </p>
        )}
        <h1
          ...
          style={{ color: maroon, fontFamily: FONTS.cinzel }}
        >
      ...
      <div
        className="flex items-start gap-6 p-5 rounded-lg mb-6 relative z-10 shadow-xs"
        style={{
      ...
      >
        ...
        <div className="flex-1 space-y-1.5 min-w-0">
          <h2
            ...
            style={{ color: maroon, fontFamily: FONTS.playfair }}
          >
            {data.fullName}
          </h2>
          {data.profileHeadline && (
            <p className="text-xs italic" style={{ color: maroon }}>
              "{data.profileHeadline}"
            </p>
          )}
          ...
          >
            <ExportFieldRow
              ...
              labelColor={maroon}
            />
            <ExportFieldRow label={lbl.height} value={data.heightFeet} labelColor={maroon} />
            <ExportFieldRow label={lbl.weight} value={data.weight} labelColor={maroon} />
            <ExportFieldRow label={lbl.bloodGroup} value={data.bloodGroup} labelColor={maroon} />
            <ExportFieldRow label={lbl.complexion} value={data.complexion} labelColor={maroon} />
            <ExportFieldRow
            ...
            <ExportFieldRow
              ...
              labelColor={maroon}
            />
            <ExportFieldRow label={lbl.subCaste} value={data.subCaste} labelColor={maroon} />
            <ExportFieldRow label={lbl.motherTongue} value={data.motherTongue} labelColor={maroon} />
            <ExportFieldRow
              label={lbl.diet}
              value={data.diet && data.diet !== 'prefer_not_to_disclose' ? data.diet.replace('_', ' ') : undefined}
              labelColor={maroon}
            />
            <ExportFieldRow label={lbl.nativePlace} value={data.nativePlace} labelColor={maroon} />
            <ExportFieldRow
      ...
      <div className="space-y-4 relative z-10 text-xs">
        ...
        {(data.fatherName || data.motherName || data.fatherOccupation || data.motherOccupation || data.brothersCount > 0 || data.sistersCount > 0 || data.brothersDetails || data.sistersDetails || data.familyType || data.familyStatus || data.familyValues || data.familyNativePlace || data.familyResidence || data.familyIntroduction || (data.additionalFamilyMembers && data.additionalFamilyMembers.length > 0)) && (
          <div data-pdf-section="family-details">
            ...
            {data.familyIntroduction && (
              <p className="mt-2 text-slate-700 pl-2 leading-relaxed italic">
                "{data.familyIntroduction}"
              </p>
            )}
        ...
        {(data.aboutMe || (data.hobbies && data.hobbies.length > 0) || data.personality || data.lifestyle || (data.languagesKnown && data.languagesKnown.length > 0)) && (
          <div data-pdf-section="about-me">
            ...
            {data.aboutMe && (
              <p className="text-slate-700 leading-relaxed pl-2 whitespace-pre-line mb-2">
                {data.aboutMe}
              </p>
            )}
            <div
        ...
        {data.includePartnerPreferences && (
          <div data-pdf-section="partner-preferences">
            ...
            {data.partnerExpectations && (
              <p className="pl-2 mt-2 text-slate-700 italic">"{data.partnerExpectations}"</p>
            )}
          </div>
        )}
